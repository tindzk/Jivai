...
def(void, ParseAttrValue) {
	...
	while (StringReader_Peek(&this->reader, &c)) {
		if (quote) {
			...
			if (c == quoteType && prev != '\\') {
				callback(this->onToken, XML_TokenType_AttrValue, value);
				break;
			}
		} else if (c == '"' || c == '\'') {
...
def(bool, ParseAttr) {
	...
	while (StringReader_Peek(&this->reader, &c)) {
		...
		if (c == '=') {
			callback(this->onToken, XML_TokenType_AttrName, name);
			StringReader_Consume(&this->reader);
		...
		{
			if (name.len != 0) {
				callback(this->onToken, XML_TokenType_Option, name);
			}
...
def(void, ParseTagStart) {
	...
	while (StringReader_Peek(&this->reader, &c)) {
		if (c == '>') {
			...
			if (commitName) {
				callback(this->onToken, XML_TokenType_TagStart, name);
			}
			break;
		} else if (StringReader_Peek(&this->reader, &str, 2) && String_Equals(str, $("/>"))) {
			/* This is an XML tag like `<br />'. */
...
def(void, ParseTagEnd) {
	...
	while (StringReader_Peek(&this->reader, &c)) {
		if (c == '>') {
			StringReader_Consume(&this->reader);
			callback(this->onToken, XML_TokenType_TagEnd, name);
			break;
		} else {
			StringReader_Extend(&this->reader, &name);
		}
...
def(void, ParseComment) {
	...
	while (!StringReader_IsEnd(&this->reader)) {
		if (StringReader_Peek(&this->reader, &str, 3) && String_Equals(str, $("-->"))) {
			...
			callback(this->onToken, XML_TokenType_Comment, comment);
			break;
		} else {
...
def(void, ParseData) {
	...
	while (!StringReader_IsEnd(&this->reader)) {
		if (StringReader_Peek(&this->reader, &str, 3) && String_Equals(str, $("]]>"))) {
			StringReader_Consume(&this->reader, 3);
			callback(this->onToken, XML_TokenType_Data, data);
			break;
		} else {
...
def(void, ParseType) {
	...
	while (StringReader_Peek(&this->reader, &c)) {
		if (c == '>') {
			StringReader_Consume(&this->reader);
			callback(this->onToken, XML_TokenType_Type, type);
			break;
		} else {
...
static def(void, Parse) {
	...
	while (StringReader_Peek(&this->reader, &c)) {
		if (!call(IsTag)) {
			...
			if (value.len != 0) {
				callback(this->onToken, XML_TokenType_Value, value);
				value.len = 0;
			}
	...
	callback(this->onToken, XML_TokenType_Done, $(""));
}
